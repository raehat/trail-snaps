...
contract ETHEscrow {
    ...
    // function sendPayment(address payable _receiver, uint256 _deadline) external payable {
    //     require(msg.sender != _receiver, "Sender and receiver cannot be the same");
    //     require(_deadline > block.timestamp, "Deadline must be in the future");
    //     require(msg.value > 0, "Payment amount must be greater than zero");

    //     currPaymentId++;
    //     Payment memory payment = Payment(payable(msg.sender), _receiver, msg.value, _deadline, false, currPaymentId);
    //     sentPayments[msg.sender][currPaymentId] = payment;
    //     receivedPayments[_receiver][currPaymentId] = payment;
    //     payments.push(payment);

    //     emit PaymentInitiated(currPaymentId, msg.sender, _receiver, msg.value, _deadline);
    // }
    ...
    currPaymentId++;
    Payment memory payment = Payment(payable(msg.sender), _receiver, msg.value, deadline, false, currPaymentId);
    sentPayments[msg.sender][currPaymentId] = payment;
    receivedPayments[_receiver][currPaymentId] = payment;
    payments.push(payment);
